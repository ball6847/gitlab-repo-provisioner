...
import { ValidateConfigurationUseCase } from '../../src/application/useCases/ValidateConfigurationUseCase.ts';
import { RepositoryConfigurationDto } from '../../src/application/dto/RepositoryDto.ts';
...
Deno.test('ValidateConfigurationUseCase - empty repositories array', () => {
  ...
  assertEquals(result.errors.length, 1);
  assertEquals(result.errors[0].field, 'repositories');
  assertEquals(result.errors[0].message, 'At least one repository must be specified');
});
...
Deno.test('ValidateConfigurationUseCase - invalid visibility', () => {
  ...
  assertEquals(result.errors[0].message, 'Visibility must be one of: private, internal, public');
});
...
Deno.test('ValidateConfigurationUseCase - missing required fields', () => {
  ...
  const pathError = result.errors.find(e => e.field === 'repositories[0].path');
  const branchError = result.errors.find(e => e.field === 'repositories[0].defaultBranch');
